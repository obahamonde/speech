...
def get_code_content_type(file_extension: str) -> dict[str, str]:
    extension_map = {
        ".py": "python",
        ".js": "javascript",
        ".ts": "typescript",
    ...
    ext = extension_map.get(file_extension.lower(), "") or "plaintext"
    return {"Content-Type": f"text/{ext}"}
...
def coalesce(args: tuple[T]) -> Union[T, None]:
    """
    Returns the first non-None value from a list of arguments.

    Args:
                    args (list[T]): List of arguments.

    Returns:
                    T: The first non-None value from the list of arguments.
    """
